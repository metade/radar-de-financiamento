...
Every normal Jekyll build also emits a public, vendor-neutral static API under `/api/v1/`:

- `/api/v1/meta.json` describes the API version, build time, and counts.
- `/api/v1/catalog.json` is the compact discovery catalogue of open or not-yet-closed opportunities.
- `/api/v1/opportunities/{id}.json` contains one detailed opportunity record.
- `/api/v1/schema.json` documents the detailed record shape.

The API is generated by `_plugins/static_api_generator.rb` from the same generated report documents used by the website. It is not a server, database, MCP server, or runtime search endpoint. Build it locally with `bundle exec jekyll build`; inspect the result in `_site/api/v1/`.

The API keeps historical opportunities as individual records when they remain in the report archive, while the catalogue contains only opportunities whose deadline is not past (or whose deadline is still unknown). Source facts, Funding Radar analysis, and authoritative links are separated in each detail record. Generated analysis is guidance; the official source remains authoritative.
