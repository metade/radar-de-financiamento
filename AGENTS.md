...
Complete local site/API build after generating a report:
...
The public API is static and vendor-neutral. It is generated by `_plugins/static_api_generator.rb` during the Jekyll build from the same report documents used by the website. Do not add a runtime API, database, MCP server, authentication, request-time search, or LLM call for API requests.

The generated public resources are `/api/v1/meta.json`, `/api/v1/catalog.json`, `/api/v1/opportunities/{id}.json`, and `/api/v1/schema.json`. The API output is written to `_site/api/v1/`; it is not a second manually maintained dataset and should not be committed as source data.

Keep opportunity IDs stable and URL-safe, preserve authoritative source URLs, and keep facts separate from Funding Radar analysis. Do not expose internal report/debug fields, cache keys, comparison-only fields, or private processing metadata. The catalogue includes opportunities with open or unknown deadlines; expired opportunities remain available as detail records when retained in the report archive.

`/api/v1/` is a public interface. Prefer additive changes within v1; breaking representation changes require a new `/api/v2/` rather than silently changing v1 semantics. Update the JSON Schema, API tests, README, and AI usage documentation when the public representation changes.
